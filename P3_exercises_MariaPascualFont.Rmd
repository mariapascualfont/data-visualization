...
exp.long <- pivot_longer(
  expressionData,
  cols = c(IFNA5, IFNA13, IFNA2, SPIN1, ZNF451, IFNA16, RASSF1, IFNW1, MSR1, MIR1976),
  names_to = "gene",
...
```{r}
ggplot(exp.long, aes(x=subject, y=gene, fill = log.expression)) +
  geom_tile()
...
The over-expressed genes in the infected group are IFNW1, IFNA5, IFNA2, IFNA16 and IFNA13.
...
manhattanPlot1 <- ggplot(gwasData) +
  geom_point(aes(x=BPcum, y=-log10(P), colour = factor(CHR)))
...
manhattanPlot2 <- manhattanPlot1 +
  scale_x_continuous(labels = axisdf$CHR, 
...
manhattanPlot3 <- 
  manhattanPlot2 + 
  labs(x = "Chromosome", y = "-log10(P)", title = "My Manhattan plot")
...
manhattanPlot4 <- manhattanPlot3 +
  geom_abline(slope = 0, intercept = -log10(sig))
...
```{r eval=F}
# Run it!
manhattanPlot7 <- manhattanPlot6 + geom_label_repel(data=gwasData[gwasData$P<sig,], aes(x = BPcum, y = -log10(P),label=as.factor(SNP), alpha=0.7), size=5, force=1.3)
manhattanPlot7
```
...
It is giving labels to the significant SNPs, using the data filtered by only those SNPs with a P value bigger than the significance. We can see how these coincide with those that are above the significance line we made before.
...
For the reduced dataset (5000 observations) we have three significant SNPs: rs89152328, rs95077576 and rs1967363.
...
Given that the p-value is 0.06, for a  threshold of 0.05 we do not have enough significance to reject the null hypothesis that the true location shift is 0. This means, statistically we can't assume there are differences between the two methods.
...
ggplot(ToothGrowth, aes(x=factor(dose), y = len)) +
  geom_boxplot() +
geom_signif(comparisons = list(c("0.5", "1")), map_signif_level=FALSE) +
  geom_signif(comparisons = list(c("0.5", "2")),   y_position = 40, map_signif_level=FALSE) +
...
ggplot(ToothGrowth, aes(x = factor(dose), y = len, fill = factor(dose))) +
  ...
  geom_signif(comparisons = list(c("0.5", "1")),
              y_position = 30, map_signif_level = FALSE) +
  geom_signif(comparisons = list(c("1", "2")),
              y_position = 36, map_signif_level = FALSE) +
  geom_signif(comparisons = list(c("0.5", "2")),
              y_position = 42, map_signif_level = FALSE) +
...
Yes, they are. We see that all p-values are statistically significant, much smaller than 0.05. So no only dose 2 has higher tooth growth rates, but also this difference is considerable statistically speaking.
</div>
