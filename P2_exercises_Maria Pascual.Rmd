...
library(ggplot2)
data(diamonds)
ggplot(data=diamonds, aes(clarity, log10(carat)) ) + 
  geom_point(aes(colour = cut), alpha = 0.5, position = 'jitter') + 
...
# Dataset: ToothGrowth
data("ToothGrowth")
p <- ggplot(data = ToothGrowth, aes(factor(dose), len)) +
  geom_boxplot(aes(fill=supp)) +
  geom_hline(yintercept = mean(ToothGrowth$len), linetype = 'longdash')
p + annotate("text", x = 1, y = 20, label = "mean")
...
The plot uses scale_fill_brewer palette 'Dark2'.
 
:::
...
::: {style="background-color:#F0F0F0"}
#####  <i class="fas fa-comment-dots"></i> Answer:
The plot uses scale_fill_brewer palette 'Oranges'.
 
:::
